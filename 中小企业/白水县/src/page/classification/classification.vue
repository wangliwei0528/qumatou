...
export default {
  ...
  methods: {
    ...
    onsubmitedit() {
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          this.$axios({
            method: "POST",
            url: "/api/admins/edit_cate/" + this.id,
    ...
    handleDelete(index, row) {
      this.$axios({
        method: "POST",
        url: "/api/admins/del_cate/" + row.id,
