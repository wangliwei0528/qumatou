...
export default {
  ...
  methods: {
    ...
    handleEdit(index, row) {
      ...
      this.$axios({
        method: "get",
        url: "/api/sgzadmin/editaera_info",        
        params: {
          id: row.id
        }
      })
    ...
    onsubmitedit(event) {
      ...
      this.$refs.formedit.validate(valid => {
        if (valid) {
          this.$axios({
            method: "POST",
            url: "/api/sgzadmin/edit_editarea/" + this.id,           
            data: formData
          })
            .then(res => {
              this.status_code = res.data.status;
    ...
    handledDel(index, row) {
      this.$axios({
        method: "post",
        url: "/api/sgzadmin/del_editarea/" + row.id,        
      })
        .then(res => {
    ...
    show(index, row) {
      ...
      this.$axios({
        method: "POST",
        url: "/api/sgzadmin/editaera_isshow/" + row.id,        
        data: {
          status: this.status
        }
      })
