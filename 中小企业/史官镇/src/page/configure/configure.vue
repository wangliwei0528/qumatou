...
export default {
  ...
  methods: {
    ...
    hideClick(index, row) {
      ...
      this.$axios({
        method: "GET",
        url: "/api/sgzadmin/config_isshow/" + row.id,       
        params: {
    ...
    deleteClick(index, row) {
      this.$axios({
        method: "POST",
        url: "/api/sgzadmin/del_config/" + row.id,        
      }).then(res => {
    ...
    submission() {
      if (this.id) {
        this.$axios({
          method: "POST",
          url: "/api/sgzadmin/edit_config/" + this.id,          
          data: {
            name: this.inputRole,
            add_type: 1
          }
